...
*Note: This method is available for future enhancements but is not currently used by the graph generator logic.*

### Helper Methods
-   `_get(endpoint)`: Performs a GET request to the specified endpoint, handling common errors like 401 Unauthorized.
-   `_post(endpoint, payload)`: Performs a POST request to the specified endpoint. It also handles common errors like 401 Unauthorized.
