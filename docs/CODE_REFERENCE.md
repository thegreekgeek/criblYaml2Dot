...
6.  **Create Edges**:
    ...
    -   **Note**: The logic relies solely on the `connections` array within the input configuration. It does not currently use the `api_client.get_pipelines()` method to fetch pipeline definitions.
...
The HTML template used to render the main page.

-   **Parameters**:
    -   `svg_content`: The raw SVG string generated by Graphviz.
-   **Structure**: A simple HTML page with a title and a container `div` that embeds the SVG content using the `|safe` filter to prevent escaping.
